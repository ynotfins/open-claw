...
1. Agent sees `ONBOARDING.md` with `status: not\_started`
...
**Memory Search:** Use semantic search (memory\_search) before answering questions about prior work, decisions, or preferences. Don't guess — search.
...
**How to monitor:** Run `session\_status` periodically during longer conversations. Look for:
...
- \[ ] Check active agents: `sessions\_list --activeMinutes 120`
...
2. During heartbeats: check `sessions\_list` for status
...
cron action=add job={
  ...
  "sessionTarget": "main",
  "schedule": {"kind": "cron", "expr": "0 14 \* \* 0", "tz": "America/Los\_Angeles"},

...
# Canonical mapping: curated packet directory -> runtime identity.
#
# Key concepts:
# - packet_dir (dict key): curated role packet folder under `AI_Employee_knowledgebase/AI_employees/`.
# - telegram_username: Telegram bot username and the canonical runtime slug used as `OPENCLAW_AGENT_ID`.
# - telegram_name: Telegram display name (rendered in audits as "Current Telegram first name").
# - secret_name: Bitwarden secret item name used to resolve credentials; this may intentionally differ from telegram_username.
# - SECRET_IDS: maps secret_name -> Bitwarden UUID to support direct secret-id wiring in startup flows.
...
- **Packet directory**: the curated employee packet folder under `open-claw/AI_Employee_knowledgebase/AI_employees/`.
- **Runtime slug**: the canonical runtime identity used as `OPENCLAW_AGENT_ID` and (typically) the Telegram bot username.
...
| Runtime slug (OpenClaw agent id / Telegram username) | Packet directory | Title | Current Telegram first name | Telegram username | Bitwarden secret name | Deployment mode | Runtime status | Current grade |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
{assignment_table}
...
def main(*, mapping_only: bool) -> None:
            ...
            json.dumps(
                {
                    ...
                    "generated_on": TODAY,
                    "employee_count": len(employees),
                    "outputs": [str(CURRENT_EMPLOYEES_PATH), str(TEAM_STATUS_PATH)],
                },
                indent=2,
            )
        )
    ...
    results = {
        ...
        "every employee has runtime files": all(
            all((EMPLOYEE_ROOT / employee.packet_dir / name).exists() for name in RUNTIME_FILES) for employee in employees
        ),
        "every employee has copied skills": all(
            all((EMPLOYEE_ROOT / employee.packet_dir / "skills" / skill / "SKILL.md").exists() for skill in employee.skills)
            for employee in employees
        ),
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Sync curated employee runtime artifacts and canonical mapping outputs.")
    parser.add_argument(
        "--mapping-only",
        action="store_true",
        help="Only regenerate current_employees.md and CURATED_TEAM_STATUS.json (no runtime shell, no zip rewrites).",
    )
    args = parser.parse_args()
    main(mapping_only=args.mapping_only)

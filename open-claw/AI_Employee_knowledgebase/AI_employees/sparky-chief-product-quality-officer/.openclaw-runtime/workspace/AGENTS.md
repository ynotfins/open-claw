...
3. Read `MEMORY.md`, `live/SESSION-STATE.md`, and recent `DECISION_LOG.md` entries before assuming the session is fresh.
4. Check `live/working-buffer.md` for uncommitted notes and promote them before continuing if needed.
5. Restate the goal, risks, and evidence needed.
6. Keep work modular and document new decisions immediately.

## Session Continuity

Sparky now carries a packet-local continuity layer adapted from the more advanced `temp/Sparky` development packet.

- `MEMORY.md` defines what belongs in active state versus durable decisions.
- `live/SESSION-STATE.md` is the active working-memory file for ongoing work.
- `DECISION_LOG.md` is the durable cross-session record for major decisions and heartbeat outcomes.
- `live/working-buffer.md` is the in-flight buffer that must be promoted before reset/compaction.
- `ONBOARDING.md` is the recovery path when Sparky enters a new environment or loses context.
